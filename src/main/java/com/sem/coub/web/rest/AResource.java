...
import com.sem.coub.web.rest.errors.BadRequestAlertException;
import java.net.URI;
...
/**
 * REST controller for managing {@link com.sem.coub.domain.A}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class AResource {
    ...
    private static final String ENTITY_NAME = "coubA";
     ...
     */
    @PutMapping("/as/{id}")
    public ResponseEntity<A> updateA(@PathVariable(value = "id", required = false) final UUID id, @Valid @RequestBody A a)
    ...
    public ResponseEntity<A> partialUpdateA(@PathVariable(value = "id", required = false) final UUID id, @NotNull @RequestBody A a)
        throws URISyntaxException {
        ...
        Optional<A> result = aRepository
            ...
            .map(existingA -> {
                if (a.getGroupId() != null) {
                    existingA.setGroupId(a.getGroupId());
                }
                if (a.getFirstName() != null) {
                    existingA.setFirstName(a.getFirstName());
                }
                if (a.getLastName() != null) {
                    existingA.setLastName(a.getLastName());
                }
                if (a.getEmail() != null) {
                    existingA.setEmail(a.getEmail());
                }
                if (a.getPhoneNumber() != null) {
                    existingA.setPhoneNumber(a.getPhoneNumber());
                }
                if (a.getHireDate() != null) {
                    existingA.setHireDate(a.getHireDate());
                }
     ...
     */
    @GetMapping("/as/{id}")
    public ResponseEntity<A> getA(@PathVariable UUID id) {
     ...
     */
    @DeleteMapping("/as/{id}")
    public ResponseEntity<Void> deleteA(@PathVariable UUID id) {
